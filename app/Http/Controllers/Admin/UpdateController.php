...
class UpdateController extends Controller
{
    ...
    {
        abort_if(Gate::denies('update_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $api = new UpdateController;
        $page_title = "";
        $update_data = $api->check_update();
        $gnl = getGen();
        return view('admin.update.index', compact('api', 'gnl', 'page_title', 'update_data'));
    }

    public function lic_activate()
    {
        abort_if(Gate::denies('license_activate'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $page_title = "";
        $api = new UpdateController;
        $gnl = getGen();
        return view('admin.update.activate', compact('api', 'gnl', 'page_title'));
    }

    public function lic_deactivate()
    {
        abort_if(Gate::denies('license_deactivate'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $page_title = "";
        $api = new UpdateController;
        $gnl = getGen();
        return view('admin.update.deactivate', compact('api', 'gnl', 'page_title'));
    }
    ...
    public function check_new_version()
    {
        ...
        if (!isset($version['latest_version'])) {
        ...
        try {
            $gnl = getGen();
            $gnl->new_version = $version['latest_version'];
            $gnl->save();
        } catch (\Throwable $th) {
            //throw $th;
        }
        ...
        return response()->json([
            'success' => true,
            'type' => $gnl ? ($gnl->new_version > $this->current_version ? 'success' : 'warning') : 'warning',
            'message' => $gnl ? ($gnl->new_version > $this->current_version ? 'New Version Released' : 'No New Releases') : 'No New Releases',
        ]);
    }
    ...
    public function activate_lic($license, $client, $create_lic = true)
    {
        $data_array =  array(
            "product_id"  => $this->product_id,
            "license_code" => $license,
