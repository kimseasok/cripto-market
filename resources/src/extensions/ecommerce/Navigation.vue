<template>
  <nav
    class="my-custom-class rounded border-gray-200 bg-white py-2.5 dark:bg-gray-900"
  >
    <div
      class="mx-auto flex flex-wrap items-start justify-start md:justify-between"
    >
      <span
        class="self-center whitespace-nowrap text-2xl font-semibold dark:text-white px-2"
        >{{ $t("Marketplace") }}</span
      >
      <div class="flex md:order-2">
        <div class="relative hidden md:block">
          <div
            class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"
          >
            <svg
              class="h-5 w-5 text-gray-500"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
                clip-rule="evenodd"
              ></path>
            </svg>
            <span class="sr-only">{{ $t("Search icon") }}</span>
          </div>
          <input
            id="search-navbar"
            v-model="ecommerceStore.search"
            type="text"
            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 pl-10 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
            placeholder="Search..."
          />
        </div>
      </div>
      <div
        id="navbar-search"
        class="w-full items-center justify-between md:w-auto md:flex md:order-1"
      >
        <div class="relative mt-3 md:hidden">
          <div
            class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"
          >
            <svg
              class="h-5 w-5 text-gray-500"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
                clip-rule="evenodd"
              ></path>
            </svg>
          </div>
          <input
            id="search-navbar"
            v-model="ecommerceStore.search"
            type="text"
            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 pl-10 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
            placeholder="Search..."
          />
        </div>
        <ul
          class="mt-4 flex sm:flex-row rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800 md:mt-0 md:space-x-8 md:border-0 md:bg-white md:text-sm md:font-medium md:dark:bg-gray-900"
        >
          <li>
            <router-link
              to="/marketplace/index"
              :class="
                $route.path == '/marketplace/index'
                  ? 'block rounded bg-blue-700 py-2 pl-3 pr-4 text-white dark:text-white md:bg-transparent md:p-0 md:text-blue-700'
                  : 'block rounded py-2 pl-3 pr-4 text-gray-700 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:p-0 md:hover:bg-transparent md:hover:text-blue-700 md:dark:hover:bg-transparent md:dark:hover:text-white'
              "
              aria-current="page"
              >{{ $t("Products") }}</router-link
            >
          </li>
          <li>
            <router-link
              to="/marketplace/wishlist"
              :class="
                $route.path == '/marketplace/wishlist'
                  ? 'block rounded bg-blue-700 py-2 pl-3 pr-4 text-white dark:text-white md:bg-transparent md:p-0 md:text-blue-700'
                  : 'block rounded py-2 pl-3 pr-4 text-gray-700 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:p-0 md:hover:bg-transparent md:hover:text-blue-700 md:dark:hover:bg-transparent md:dark:hover:text-white'
              "
              >{{ $t("Wishlist") }}</router-link
            >
          </li>
          <li>
            <router-link
              to="/marketplace/order-history"
              :class="
                $route.path == '/marketplace/order-history'
                  ? 'block rounded bg-blue-700 py-2 pl-3 pr-4 text-white dark:text-white md:bg-transparent md:p-0 md:text-blue-700'
                  : 'block rounded py-2 pl-3 pr-4 text-gray-700 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:p-0 md:hover:bg-transparent md:hover:text-blue-700 md:dark:hover:bg-transparent md:dark:hover:text-white'
              "
              >{{ $t("Orders") }}</router-link
            >
          </li>
        </ul>
      </div>
    </div>
  </nav>
</template>

<script>
  import { useEcommerceStore } from "@/store/ecommerce";
  export default {
    setup() {
      const ecommerceStore = useEcommerceStore();
      return { ecommerceStore };
    },
  };
</script>

<style scoped>
  .my-custom-class {
    @media (max-width: 599px) {
      .container {
        max-width: 600px;
      }
    }
  }
</style>
