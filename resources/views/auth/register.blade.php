<x-guest-layout>
    ...
    <section id="register-page">
        <div class="grid h-screen place-items-center max-w-screen-xl px-4 py-8 mx-auto lg:gap-20 lg:py-16 lg:grid-cols-12">
            <div class="w-full p-6 mx-auto rounded-lg dark:bg-gray-800 sm:max-w-xl lg:col-span-6 sm:p-8">
                <div>
                    <img src="/assets/images/logoIcon/logo-dark.png" alt="logo" class="logo mb-5 px-3">
                </div>
                ...
                <form class="mt-4 space-y-6 sm:mt-6" method="POST" id="registerForm" action="{{ route('register') }}">
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            <div>
                                <x-jet-label for="firstname" value="{{ __('Firstname') }}" class="hidden" />
                                <x-jet-input id="firstname" class="block mt-1 w-full" type="text" name="firstname" :value="old('firstname')" required autofocus autocomplete="given-name" placeholder="Firstname" />
                            </div>
                        ...
                        <div>
                            <div>
                                <x-jet-label for="lastname" value="{{ __('Lastname') }}" class="hidden" />
                                <x-jet-input id="lastname" class="block mt-1 w-full" type="text" name="lastname" :value="old('lastname')" required autocomplete="family-name" placeholder="Lastname" />
                            </div>
                        ...
                        <div>
                            <div>
                                <x-jet-label for="username" value="{{ __('Username') }}" class="hidden" />
                                <x-jet-input id="username" class="block mt-1 w-full" type="text" name="username" :value="old('username')" required autocomplete="username" placeholder="Username" />
                            </div>
                        ...
                        <div>
                            <div>
                                <x-jet-label for="referal" value="{{ __('Referal') }}" class="hidden" />
                                <x-jet-input id="referal" class="block mt-1 w-full" type="text" name="referal" :value="old('referal')" required autocomplete="username" placeholder="Referal" />
                            </div>
                            @error('referal', 'register')
                            <span class="mt-2 text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            <div>
                                <x-jet-label for="email" value="{{ __('Email') }}" class="hidden" />
                        ...
                        <div>
                            <div>
                                <x-jet-label for="password" value="{{ __('Password') }}" class="hidden" />
                        ...
                        <div>
                            <div>
                                <x-jet-label for="password_confirmation" value="{{ __('Confirm Password') }}" class="hidden" />
                        ...
                        <div>
                            ...
                            <select id="country" name="country" placeholder="Country" aria-describedby="country" class="form-control" onchange="updatePhoneNumberPrefix();">
                                @php
                                $visitorCountry = getVisitorCountry(request());
                                @endphp
                                @foreach (getCountries() as $country)
                                <option value="{{ $country }}" {{ $country == $visitorCountry ? 'selected' : '' }}>@lang($country)
                                </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <div>
                                <div class="flex">
                                    <input id="phonePrefix" name="country_code" class="form-control" type="text" style="width: 60px !important;" disabled />
                                    <x-jet-input id="phone" class="block w-full ml-2" type="tel" name="phone" pattern="\d{1,4}[\s-]?\d{1,6}" placeholder="E.g. 123-456-7890" required />
                                </div>
                    ...
                    <div id="registerBtn">
                        <x-jet-button class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
            ...
            <div class="mr-auto place-self-center lg:col-span-6">
                <img class="hidden mx-auto lg:flex scale-125" src="../assets/images/illustrations/register.svg" alt="illustration">
            </div>
        </div>
        <br class="clearboth">
    </section>
